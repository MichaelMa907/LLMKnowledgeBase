...
- Commands / tools: read the local Codex instruction entrypoint, attempted reads for local Codex sidecar instruction files, listed the local Codex config directory, reread `AGENTS.md` and `CLAUDE.md`, patched markdown notes, and ran the then-current refresh script.
- Files / notes touched: root `AGENTS.md`, `Projects/llm-knowledge-base-system/llm-knowledge-base-system.md`, this commit note, a local machine-scoped Codex entrypoint note, and regenerated index files.
...
- Files / notes touched: `AGENTS.md`, `CLAUDE.md`, `Templates/*`, `Scripts/update-knowledge-base.mjs`, then-current global learned notes, local machine-scoped learned notes, this commit note, and the project note.
...
- Machine notes created or updated:
  Local machine-scoped runtime and instruction-entrypoint notes under `Machines/<machine-id>/Learned/`
